# classifier_app.py - Complete AI Incident Type Classifier for Streamlit Cloud
...
with st.spinner("Fetching data from Supabase..."):
    ...
    if response.data:
        ...
        st.success(f"✅ Loaded {len(df)} records from cyber_news table")
        return df
    else:
        st.warning("⚠️ cyber_news table has no data")
        return pd.DataFrame()
...
st.caption("🔐 Credentials are read from Streamlit Cloud Secrets (SUPABASE_URL and SUPABASE_KEY)")
...
Please check:
1. Your Supabase table is named exactly 'cyber_news'
