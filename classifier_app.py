# classifier_app.py - Complete standalone application
...
class SupabaseAdapter:
    def __init__(self):
        # Try to get from environment variables (Streamlit Cloud secrets)
        self.url = os.environ.get('SUPABASE_URL')
        self.key = os.environ.get('SUPABASE_KEY')
        
        # Also try Streamlit secrets (alternative method)
        if not self.url and hasattr(st, 'secrets'):
            try:
        ...
        if not self.url or not self.key:
            ...
            Please set them in Streamlit Cloud secrets or environment variables:
            - SUPABASE_URL
            - SUPABASE_KEY
            """)
            self.client = None
    ...
    def fetch_cyber_news(self, limit=None):
...
class IncidentClassifier:
    ...
    def train(self, df):
        ...
        if df.empty:
            st.error("No valid incident_type labels found")
            return False
        
        # Label mapping (consolidate into standard categories)
        label_map = {
            # Intrusion System
        ...
        if df.empty:
            st.error("Not enough samples per class (need at least 10 per class)")
            return False
...
def _render_live_classifier(clf: IncidentClassifier):
    ...
    with st.expander("📌 Try these examples"):
        examples = [
            ('🔴 Intrusion System', 'LockBit ransomware group claims attack on Malaysian government ministry demanding payment'),
        ...
        for emoji, ex in examples:
            if st.button(f'{emoji} {ex[:70]}...', key=ex):
                result = clf.predict_one(ex)
                st.markdown(f"<span style='color:{SEVERITY_COLOUR.get(result['label'], C['accent'])};font-weight:700;'>{result['label']}</span> ({result['confidence']*100:.1f}% confidence)", unsafe_allow_html=True)
...
def main():
    ...
    with st.sidebar:
        ...
        st.markdown('---')
        st.caption("💡 The model trains automatically on first load. Click Retrain after adding new incidents.")

    # Load data from Supabase
    with st.spinner('Loading incident data from Supabase...'):
        adapter = SupabaseAdapter()
        df = adapter.fetch_cyber_news()
        
        if df.empty:
            st.error("""
            ⚠️ **No data available in cyber_news table**
            
            Please check:
            1. Your Supabase connection is working
            2. The table name is 'cyber_news'
            3. The table has data
            4. Environment variables SUPABASE_URL and SUPABASE_KEY are set correctly
            """)
            return
        
        st.success(f"✅ Loaded {len(df)} incidents from cyber_news table")

    # Train or load model
    cache_key = f'clf_{len(df)}'
    
    if retrain or cache_key not in st.session_state:
        with st.spinner('🧠 Training MLP classifier on cyber_news data... This may take a moment...'):
            clf = IncidentClassifier()
            if clf.train(df):
                st.session_state[cache_key] = clf
                st.success('✅ Model trained successfully!')
            else:
                st.error('❌ Model training failed. Please check your data format.')
                return
    else:
