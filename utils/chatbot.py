...
# ─────────────────────────────────────────────────────────────
#  ThreatBook API helpers (unchanged from original)
# ─────────────────────────────────────────────────────────────
...
def _tb_ip(ip: str, api_key: str) -> dict:
    ...
    for method, url in endpoints:
        try:
            r = requests.request(method, url, params={"apikey": api_key, "resource": ip}, timeout=TB_TIMEOUT)
            ...
            if rc in (401, 405):
                last_err = {"error": f"API key has no access to {url} ({rc})"}
                continue
            return data
        except Exception as e:
...
def _tb_domain(domain: str, api_key: str) -> dict:
    ...
    for method, url in endpoints:
        try:
            r = requests.request(method, url, params={"apikey": api_key, "resource": domain}, timeout=TB_TIMEOUT)
            data = r.json() if r.status_code == 200 else {}
            if not data:
                last_err = {"error": r.text[:200]}
                continue
...
def _tb_file(hash_val: str, api_key: str) -> dict:
    try:
        r = requests.post(f"{TB_BASE}/v2/file/report", params={"apikey": api_key, "resource": hash_val}, timeout=TB_TIMEOUT)
...
def _tb_url(url_val: str, api_key: str) -> dict:
    try:
        r = requests.post(f"{TB_BASE}/v2/url/report", params={"apikey": api_key, "resource": url_val}, timeout=TB_TIMEOUT)
        return r.json() if r.status_code == 200 else {"error": r.text[:200]}
    except Exception as e:
        return {"error": str(e)}
...
# ─────────────────────────────────────────────────────────────
#  IOC extractor (unchanged from original)
# ─────────────────────────────────────────────────────────────
...
# ─────────────────────────────────────────────────────────────
#  ThreatBook enrichment (unchanged from original)
# ─────────────────────────────────────────────────────────────
...
def _summarise_ip_report(data: dict) -> str:
    ...
    if rc is not None and rc != 0:
        return f"(ThreatBook error {rc}: {data.get('msg', 'unknown error')})"
    d = data.get("data", {})
        ...
        if not dated.empty:
            try:
                lines.append(
                    ...
                    f"to {dated['incident_date'].max().date()}"
                )
            except Exception:
                pass
...
def chatbot_ui(df: pd.DataFrame):
    ...
    #flocks-chat-panel .gcp-header {
        background: linear-gradient(135deg,#161b22,#1c2128);
        border-bottom: 1px solid #21262d;
        padding: 14px 16px; display: flex; align-items: center;
        gap: 10px; flex-shrink: 0;
    }
    #flocks-chat-panel .gcp-avatar {
        width:34px; height:34px;
        background: linear-gradient(135deg,#1a7f64,#3fb950);
        border-radius:50%; display:flex; align-items:center;
        justify-content:center; font-size:16px; flex-shrink:0;
    }
    #flocks-chat-panel .gcp-title  { font-weight:600; font-size:14px; color:#f0f6fc; }
    #flocks-chat-panel .gcp-sub    { font-size:11px; color:#8b949e; margin-top:1px; }
    .gcp-tb-badge {
    ...
    #flocks-chat-panel .gcp-body {
        flex:1; overflow-y:auto; padding:14px;
        display:flex; flex-direction:column; gap:10px;
        scrollbar-width:thin; scrollbar-color:#30363d transparent;
    }
    #flocks-chat-panel .gcp-body::-webkit-scrollbar { width:4px; }
    #flocks-chat-panel .gcp-body::-webkit-scrollbar-thumb { background:#30363d; border-radius:4px; }
    ...
    .gc-msg.bot  .gc-role { color:#3fb950; }
    ...
    #flocks-chat-panel .stMarkdown p { margin: 0 0 6px 0; color: #c9d1d9; font-size: 13px; line-height: 1.7; }
    #flocks-chat-panel .stMarkdown h3 { font-size: 12px; font-weight: 700; color: #f0f6fc;
        text-transform: uppercase; letter-spacing: .08em; margin: 12px 0 4px;
        border-bottom: 1px solid #21262d; padding-bottom: 4px; }
    #flocks-chat-panel .stMarkdown ul, #flocks-chat-panel .stMarkdown ol
        { padding-left: 16px; margin: 4px 0 8px; color: #c9d1d9; font-size: 13px; }
    #flocks-chat-panel .stMarkdown li { margin-bottom: 3px; line-height: 1.6; }
    #flocks-chat-panel .stMarkdown strong { color: #f0f6fc; font-weight: 600; }
    #flocks-chat-panel .stMarkdown code
    ...
    #flocks-chat-panel .stChatInput { border:none !important; background:transparent !important; }
    #flocks-chat-panel .stChatInput > div {
        background:#161b22 !important; border:1px solid #30363d !important;
        border-radius:20px !important;
    }
    #flocks-chat-panel .stChatInput textarea { color:#c9d1d9 !important; font-size:13px !important; }
    #flocks-chat-panel .stChatInput button {
        background: linear-gradient(135deg,#1a7f64,#3fb950) !important;
    ...
    .gc-dot { width:6px; height:6px; background:#3fb950; border-radius:50%;
    ...
    # ── FAB button ─────────────────────────────────────────────
    st.markdown('<div id="flocks-fab-anchor">', unsafe_allow_html=True)
    msg_count = len(st.session_state.chat_history)
    badge = f" ({msg_count})" if msg_count > 0 and not st.session_state.chat_open else ""
    fab_label = "✕ Close" if st.session_state.chat_open else f"🛡️ CTI Analyst{badge}"

    if st.button(fab_label, key="flocks_fab_btn", help="Toggle Chatbot"):
    ...
    #flocks-fab-anchor button {
        ...
        box-shadow:0 4px 20px rgba(26,127,100,0.45) !important;
    ...
    #flocks-fab-anchor button:hover {
        transform:scale(1.05) !important;
        box-shadow:0 6px 28px rgba(26,127,100,0.6) !important;
    }
    #flocks-fab-anchor > div { margin:0 !important; }
    </style>
    """, unsafe_allow_html=True)
    ...
    # ── Chat panel ─────────────────────────────────────────────
    st.markdown('<div id="flocks-chat-panel">', unsafe_allow_html=True)
    ...
    if not st.session_state.chat_history:
        ...
        for msg in st.session_state.chat_history:
            ...
            if role == "user":
                ...
                with st.container():
                    st.markdown("<div class='gc-bot-md'></div>", unsafe_allow_html=True)
    ...
    user_input = st.chat_input("Ask analyst or paste IP/domain/hash/CVE…", key="flocks_chat_input")
    st.markdown('</div>', unsafe_allow_html=True)
    st.markdown('</div>', unsafe_allow_html=True)
